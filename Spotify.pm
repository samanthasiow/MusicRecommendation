...
has 'oauth_client_id' => (
    is      => 'rw',
    isa     => 'Str',
    default => $ENV{'2ee3b6f7fa4f43aaaee447aa3fc3ef43'} || ''
);

has 'oauth_client_secret' => (
    is      => 'rw',
    isa     => 'Str',
    default => $ENV{'eb08103e002e4b9e92319f55e24c0980'} || ''
);
...
sub send_get_request {
    ...
    # added because 'user_playlist' wasn't recognized in previous
    # code.
    if ( $attributes->{method} eq "user_playlist") {
...
# gets the playlist given a user id and a playlist id
...
# gets the tracks from a playlist given a user id and a playlist id
